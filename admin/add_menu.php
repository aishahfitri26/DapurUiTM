...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $res_id = isset($_POST['res_id']) && !empty($_POST['res_id']) ? $_POST['res_id'] : 1;
    ...
    if (isset($_FILES["menu_image"])) {
        ...
        if ($check !== false) {
            if (move_uploaded_file($_FILES["menu_image"]["tmp_name"], $target_file)) {
                ...
                $stmt = $conn->prepare("INSERT INTO menu (c_id, menu_name, menu_price, menu_description, menu_image, menu_date, res_id) VALUES (?, ?, ?, ?, ?, ?, ?)");
                $stmt->bind_param("issssii", $c_id, $menu_name, $menu_price, $menu_description, $target_file, $menu_date, $res_id);
                $stmt->execute();
...
<label for="res_id">Restaurant Name:</label>
